...
<body>
    ...
    $dimBranch_branchID = 100535;
    $dimBranch_branchName = "DEF";
    $dimBranch_branchLocation = "DEF";
    $dimBranch_numOfRooms = 60;
    $dimBranch_contact = "DEF";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            // echo "<br>";
            // echo date($dimDate_date[$index]);
            // echo " ";
            // echo date('d', strtotime(date($dimDate_date[$index])));
            // echo date('m', strtotime(date($dimDate_date[$index])));
            // echo date('Y', strtotime(date($dimDate_date[$index])));
            // echo "<br>";
            ...
            // $conn->close();
            // include("dw.php");
            // $sql = "INSERT INTO facttable(GuestID, RoomID, BranchID, Date, Total_Amount) VALUES('$row[GuestID]', '$row[RoomID]', 'Akama_1','$row[Check_in]','$row[Amount]')";
            // echo "<tr><td>" . $row["GuestID"] . "</td><td>" . $row["RoomID"] . "</td><td>" . $branch_id . "</td><td>" . $row["Check_in"] . "</td><td>" . $row["Amount"] . "</td></tr>";
            // $result = mysqli_query($conn, $sql);
            // $conn->close();
    ...
    // $udimRoom_roomID = array_values(array_unique($dimRoom_roomID));
    ...
    // for ($i = 0; $i < sizeof($dimRoom_roomID);) {
    //     echo $dimRoom_roomID[$i];
    //     echo " ";
    //     $i += 1;
    // }
    // echo "<br>";

    // for ($i = 0; $i < sizeof($dimRoom_roomType);) {
    //     echo $dimRoom_roomType[$i];
    //     echo " ";
    //     $i += 1;
    // }
    // echo "<br>";

    // for ($i = 0; $i < sizeof($dimRoom_occupancy);) {
    //     echo $dimRoom_occupancy[$i];
    //     echo " ";
    //     $i += 1;
    // }
    // echo "<br>";

    // for ($i = 0; $i < sizeof($dimRoom_pricePerNight);) {
    //     echo $dimRoom_pricePerNight[$i];
    //     echo " ";
    //     $i += 1;
    // }
    // echo "<br>";
    // $udimRoom_roomType = array_values(array_unique($dimRoom_roomType));
    // for ($i = 0; $i < sizeof($udimRoom_roomType);) {
    //     echo $udimRoom_roomType[$i];
    //     echo " ";
    //     $i += 1;
    // }
    // echo "<br>";
    // $udimRoom_occupancy = array_values(array_unique($dimRoom_occupancy));
    // $udimRoom_pricePerNight = array_values(array_unique($dimRoom_pricePerNight));

    // array_unique($dimRoom_roomType);
    // array_unique($dimRoom_occupancy);
    // array_unique($dimRoom_pricePerNight);
    ...
    while ($index_insert < $index) {
        $sql = "INSERT INTO facttable(GuestID, RoomID, BranchID, Date, Total_Amount) VALUES('$store_guestID[$index_insert]', '$store_RoomID[$index_insert]', '$branch_id','$store_guestDate[$index_insert]',' $store_amount[$index_insert]')";
        $result = mysqli_query($conn, $sql);
    ...
    // include("database2.php");
    // $sql = "SELECT g.GuestID, r.RoomID, g.Name, g.Phone, r.Check_in, r.Check_out, ri.RoomType, ri.PricePerNight, g.Amount FROM reservation r inner join guest g on g.GuestID = r.GuestID inner join roominfo ri on r.RoomID = ri.RoomID order by r.GuestID";
    // $result = mysqli_query($conn, $sql);
    // $branch_id = "Akama_1";
    ...
    // $index_insert = 0;
    // while ($index_insert < $index) {
    //     $sql = "INSERT INTO facttable(GuestID, RoomID, BranchID, Date, Total_Amount) VALUES('$store_guestID[$index_insert]', '$store_RoomID[$index_insert]', 'Akama_2','$store_guestDate[$index_insert]',' $store_amount[$index_insert]')";
    //     $result = mysqli_query($conn, $sql);
    //     $index_insert += 1;
    // }
