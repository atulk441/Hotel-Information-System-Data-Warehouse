    ...
    <div class="container">
        ...
        <div class="item_right">
            <div class="button-container">
                <button class="top-right-button" onclick="window.location.href = 'sdb2v2.php';">Go Back</button>
            </div>
            ...
            <div class="sub_container">
                <form action="insertdatasdb2.php" method="post">
                    ...
                    <div class="form-group new_pos">
                        <select name="room_selector" class="form-group" id="room">
                            ...
                            <option value="Personal Room">Personal Room (Max: 1 person)</option>
                            <option value="Premium Room">Premium Room (Max: 2 persons)</option>
                            <option value="Suite Room">Suite Room (Max: 4 persons)</option>
                        </select>
                    </div>
                    <br>
            ...
            $guest_id = file_get_contents('sdb2GuestID.txt');
            $status = 0;
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                if (
                    ...
                    $room_id = -1;
                    $sql =  "select rii.RoomNo from roominfo rii where rii.RoomNo not in
    (select distinct(ri.RoomNo) from roominfo ri join reservation r on r.RoomNo = ri.RoomNo where ri.RoomSize='$room_type' and r.Checkout > now() ) and rii.RoomSize='$room_type';";
                    $result = mysqli_query($conn, $sql);
                    if ($result->num_rows > 0) {
                        while ($row = $result->fetch_assoc()) {
                            $room_avail[$index] =  $row["RoomNo"];
                            $index += 1;
                        }
                    ...
                    if (($room_type == "Personal Room" && $num_of_guests > 1) || ($room_type == "Premium Room" && $num_of_guests > 2) || $room_type == "Suite Room" && $num_of_guests > 4) {
                        echo "<h3 style='color: red; text-align:center; margin:  45px 0px 0px 208px;'>Personal room can only accomodate maximum of 1 person.</h3> <h3 style='color: red;text-align:center;margin: 0px 0px 0px 208px;'>Premium room can only accomodate maximum of 2 persons.</h3> <h3 style='color: red;text-align:center;margin: 0px 0px 0px 208px;'>Suite room can only accomodate maximum of 4 person.</h3>";
                    } else {
            ...
            inner join guest g on g.GuestID = r.GuestID inner join roominfo ri on r.RoomNo = ri.RoomNo inner join roomdetails rd on ri.RoomSize = rd.RoomSize
            set g.amountpaid = datediff(r.checkout, r.checkin)*rd.Price;";
                        mysqli_query($conn, $sql);
                        $status = 1;
                        $sql = "Select AmountPaid from guest where guestid = '$guest_id';";
                        $fetched_res = mysqli_query($conn, $sql);
                        if ($fetched_res && $fetched_res->num_rows > 0) {
                            $row = $fetched_res->fetch_assoc();
                            $updatedAmount = $row["AmountPaid"];
                        }
                    }
                    $conn->close();
                    ...
                    if ($status == 1) {
                        include("dw.php");
                        $sql = "INSERT INTO facttable VALUES ('$guest_id', '$room_id', 'Akama_2', '$check_in', '$updatedAmount')";
                        mysqli_query($conn, $sql);
                        ...
                        if (mysqli_num_rows($result) > 0) {
                            ...
                            if ($room_type = "Personal Room") {
                                $sql = "INSERT INTO dim_room VALUES ('$room_id', '$room_type', 1, 50)";
                                mysqli_query($conn, $sql);
                            } elseif ($room_type = "Premium Room") {
                                $sql = "INSERT INTO dim_room VALUES ('$room_id', '$room_type', 2, 100)";
                                mysqli_query($conn, $sql);
                            } elseif ($room_type = "Suite Room") {
                                $sql = "INSERT INTO dim_room VALUES ('$room_id', '$room_type', 4, 150)";
                                mysqli_query($conn, $sql);
                            }
                        }
                        ...
                        file_put_contents('sdb2GuestID.txt', $guest_id);
